...
# Step 20: Create Kubernetes RBAC resources
resource "kubernetes_role" "postgresql" {
  count = var.cluster_settings.create && var.postgresql_settings.create && var.postgresql_backup.enabled ? 1 : 0

  metadata {
    name      = "volumesnapshots-role"
    namespace = kubernetes_namespace.postgresql[0].metadata[0].name
  }
...
resource "kubernetes_role_binding" "postgresql" {
  count = var.cluster_settings.create && var.postgresql_settings.create && var.postgresql_backup.enabled ? 1 : 0

  metadata {
    name      = "volumesnapshots-role-binding"
    namespace = kubernetes_namespace.postgresql[0].metadata[0].name
  }

  subject {
    kind      = "ServiceAccount"
    name      = kubernetes_service_account.postgresql[0].metadata[0].name
    namespace = kubernetes_namespace.postgresql[0].metadata[0].name
  }

  role_ref {
    kind      = "Role"
    name      = kubernetes_role.postgresql[0].metadata[0].name
...
# Step 21: Create Kubernetes cronjob
resource "kubernetes_cron_job_v1" "postgresql" {
  count = var.cluster_settings.create && var.postgresql_settings.create && var.postgresql_backup.enabled ? 1 : 0

  metadata {
    name      = var.postgresql_backup.name
    namespace = kubernetes_namespace.postgresql[0].metadata[0].name
  }

  spec {
    schedule = var.postgresql_backup.cronjob_schedule
    ...
    job_template {
      ...
      spec {
        template {
          ...
          spec {
            service_account_name = kubernetes_service.postgresql[0].metadata[0].name

            container {
              name  = "backup-container"
              image = "gcr.io/${var.project}/${var.postgresql_backup.image}:${var.postgresql_backup.version}"

              env {
                name  = "PVC_NAME"
                value = kubernetes_persistent_volume_claim.postgresql[0].metadata[0].name
              }

              env {
                name  = "NAMESPACE"
                value = kubernetes_namespace.postgresql[0].metadata[0].name
              }

              env {
                name  = "MAX_RETENTION_DAYS"
                value = var.postgresql_backup.max_retention_days
              }

              command = ["/bin/sh"]
              args    = ["-c", "python /app/snapshot.py"]
            }
...
# Step 22: Create disk snapshot policy
resource "google_compute_resource_policy" "postgresql" {
  count = var.cluster_settings.create && var.postgresql_settings.create && var.postgresql_backup.enabled ? 1 : 0
  ...
  snapshot_schedule_policy {
    schedule {
      daily_schedule {
        days_in_cycle = 1
        start_time    = var.postgresql_backup.start_time
      }
    }

    retention_policy {
      max_retention_days    = var.postgresql_backup.max_retention_days
...
# Step 23: Attach disk snapshot policy
resource "google_compute_disk_resource_policy_attachment" "postgresql" {
  count = var.cluster_settings.create && var.postgresql_settings.create && var.postgresql_backup.enabled ? 1 : 0

  name = google_compute_resource_policy.postgresql[0].name
  disk = google_compute_disk.postgresql[0].name
  zone = local.location

  depends_on = [
    google_compute_disk.postgresql,
    google_compute_resource_policy.postgresql,
  ]
}
